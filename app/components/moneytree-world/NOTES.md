...
| `moneyLessonLine(result.year)` (from `insights.ts`, not `coach.ts`) | `EventCard.tsx` | Every resolved year - a rotating financial-literacy fact independent of what happened this specific year, distinct from `yearInsight`'s `smartMove` |
| `coachAdviceLine(mascot, result.allocationWeights)` | `EventCard.tsx` | Every resolved year - the coach's own forward-looking opinion on the split just used (persona × safe-heavy/balanced/moonshot-heavy shape), distinct from `eventReactionLine` (reacts to outcome) and `allocationCoachLine` (live risk warning) |
...
- **`EventCard.tsx`** — same in-tree overlay pattern as CashOutPanel. Shows `yearInsight` (title/tone/whatHappened/smartMove), per-bucket returns, a rotating `moneyLessonLine` (financial-literacy fact, independent of this year's event/calm framing), `eventReactionLine`, a persona-flavored `coachAdviceLine` (forward-looking opinion on the split just used), `sendOffLine` (if not final), tree total, continue button. Body scrolls (`overflowY:auto`, header pinned) since this is now a fair amount of content on a small screen. Master prompt's target: becomes weather + coach dialogue, no card - the world must find room for the same five distinct pieces of content (what happened, per-bucket returns, the tip, the lesson, the coach's opinion), not just the original three.
